...
//   }, [data.interactions]);
  ...
  useEffect(() => {
    ...
    //   // --- NEW: Parquet Path Handler ---
    //   // If the data is just a file path, we bypass the legacy interaction 
    //   // indexing and simply pass the Parquet path forward.
  ...
  }, [data.interactions, output.content]);
...
//   }, [output, tabData, activeTab, createTableData]);
  const tableData = useMemo(() => {
    // Since 'output' now strictly holds the Parquet path wrapper, we must
    // exclusively use 'tabData' (populated by processDataAsync) for rendering the UI.
    const displayTable = tabData[parseInt(activeTab)];
    if (displayTable) return createTableData(displayTable as ICodeDataContent);
    return [];
